...
def get_database_url():
    ...
    if "sslmode=" not in database_url:
        seperator = "g" if "?" in database_url else "?"
...
Flight Results:
{str(state.get('flight_results', ''))[:2000]}

Hotel Results:
{str(state.get('hotel_results', ''))[:2000]}
...
Budget Results:
{str(state.get('budget_results', ''))[:2000]}
...
## final agent - original format kept, HITL feedback added
...
Weather:
{str(state.get('weather_results', ''))[:2000]}

Budget Analysis:
{str(state.get('budget_results', ''))[:2500]}

Draft Itinerary:
{str(state.get('itinerary', ''))[:3000]}
